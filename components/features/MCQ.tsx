        ...
        <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-700">
            ...
            <div className="space-y-2">
                {mcq.options.map((option, i) => {
                    ...
                    if (isAnswered) {
                        if (isCorrect) {
                           optionClass = "bg-green-800/70"; // Always highlight correct answer
                        } else if (isSelected) {
                           optionClass = "bg-red-800/70"; // Highlight user's wrong choice
                        } else {
                           optionClass = "bg-slate-700 opacity-50"; // Fade out other incorrect options
                        }
                    }
...
export const MCQ: React.FC = () => {
    ...
    const handleAnswer = (questionIndex: number, answer: string) => {
        ...
        // If this is the last answer, finish the quiz and save history
        if (mcqs.length > 0 && Object.keys(newAnswers).length === mcqs.length) {
            ...
            setHistory(prev => [newAttempt, ...prev].slice(0, 20)); // Keep last 20 attempts
        }
    };
